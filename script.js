...
const weeklyFrameworks = {
  ...
  2: {
    ...
    sunday: {
      ...
      phases: ["activation", "sit", "cooldown"],
    }, // CHANGED from Rest or Mobility/Core
  },
  3: {
    monday: {
    ...
    friday: {
      ...
      duration: { standard: 30, quick: 20, extended: 40 },
      phases: ["mobility", "cooldown"],
    }, // CHANGED from Power + Integration
    saturday: {
...
function showMotivationalMoment(phase) {
  // Different quote sets for different phases
  ...
  // Select appropriate quote based on phase
  let randomComment;
  if (phase === "activation" || phase === "jump") {
    ...
    // Fallback for mobility or any other phase
    randomComment =
      activationJumpQuotes[
        Math.floor(Math.random() * activationJumpQuotes.length)
      ];
...
function getPhaseTargetTime(phase) {
  ...
  // Adjust based on selected duration
  const durationMultipliers = {
    quick: 0.7, // 70% of standard
    standard: 1.0, // 100% (no change)
    extended: 1.3, // 130% of standard
  };
